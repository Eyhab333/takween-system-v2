...
function toDocument(
  ...
  return {
    ...
    createdByUid:
      typeof value.createdByUid ===
      "string"
        ? value.createdByUid
        : null,
    updatedByUid:
      typeof value.updatedByUid ===
      "string"
        ? value.updatedByUid
        : null,
  };
...
// يعتبر المستخدم موظفًا نشطًا إذا كان يحمل staff
// ولم يكن معطلاً أو غير نشط.
function isActiveStaffUser(
  user: Record<string, unknown>,
) {
  return (
    Array.isArray(user.tags) &&
    user.tags.includes("staff") &&
    user.active !== false &&
    user.disabled !== true &&
...
/**
 * مهم:
 * هذه الدالة أصبحت تُستدعى مرة واحدة فقط
 * في GET الخاص بقائمة المستندات.
 */
async function getTargetEmployees(): Promise<
  TargetEmployee[]
> {
  const { db } =
    getAdminServices();
...
/**
 * تقرأ فقط إقرارات الموظفين المستهدفين
 * للمستند المحدد.
 */
async function getAcknowledgedUids(
  employees: Array<{
  ...
  const acknowledgementRefs =
    employees.map((employee) =>
      db.doc(
        `users/${employee.uid}/jobComplianceAcknowledgements/${documentId}`,
      ),
    );
  ...
  for (const acknowledgement of snapshots) {
    ...
    const data =
      acknowledgement.data() as Record<
        string,
        unknown
      >;
...
/**
 * employees هنا جاهزون بالفعل من Firestore،
 * وبالتالي لا نعيد قراءة users لكل مستند.
 */
async function getDocumentProgress(
  ...
  if (
    ...
    return {
      totalTargetEmployees: 0,
      acknowledgedCount: 0,
      pendingCount: 0,
    };
  }

  const acknowledgedUids =
    await getAcknowledgedUids(
      targetEmployees,
      document.id,
      document.version,
    );
  ...
  const acknowledgedCount =
    targetEmployees.reduce(
      (count, employee) =>
        acknowledgedUids.has(
          employee.uid,
        )
          ? count + 1
          : count,
...
export async function GET(
  ...
  try {
    ...
    /**
     * نقرأ المستندات والموظفين بالتوازي.
     *
     * employees تُقرأ مرة واحدة فقط،
     * وليس مرة لكل مستند.
     */
    const [
      documentsSnapshot,
    ...
    /**
     * هنا لا توجد أي قراءة جديدة لـ users.
     *
     * فقط نحسب الجمهور من employees الموجودين
     * ثم نقرأ إقرارات المستهدفين فعليًا.
     */
    ...
    console.error(
      "admin job-compliance GET error:",
      error,
    );
  ...
  try {
    ...
    if (
      ...
      return Response.json(
        {
          error:
            "حجم ملف PDF يجب ألا يتجاوز 15 ميجابايت",
        },
        {
          status: 400,
        },
    ...
    if (!bucketName) {
      return Response.json(
        {
          error:
            "إعدادات مساحة تخزين الملفات غير مكتملة",
        },
        {
          status: 500,
        },
    ...
    const storagePath =
      `job-compliance/${documentRef.id}-${sanitizeFileName(
        file.name,
      )}`;
    ...
    console.error(
      "admin job-compliance POST error:",
      error,
    );
