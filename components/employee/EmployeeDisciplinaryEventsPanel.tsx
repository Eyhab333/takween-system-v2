...
(async () => {
  try {
    ...
    const res = await fetch(`/api/disciplinary-events?${params.toString()}`, {
      headers: {
...
return ( <div className="grid gap-4"> <Card> <CardHeader> <CardTitle>بيانات الموظف</CardTitle> </CardHeader> <CardContent className="grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-4">
...
/>
<InfoItem label="البريد الإلكتروني" value={employee?.email || "—"} /> <InfoItem label="الراتب" value={formatCurrency(employee?.salary)} /> <InfoItem
         label="الراتب اليومي"
...
return ( <div className="min-w-0"> <div className="text-xs text-muted-foreground">{label}</div> <div className="break-words font-medium">{value}</div> </div>
);
}
...
return (
<Card className={cn("rounded-xl border p-4 shadow-sm", className)}> <CardContent className="p-1"> <div className="text-xs text-muted-foreground">{title}</div> <div className="mt-2 break-words text-xl font-bold text-violet-900">
...
if (t.includes("مخالفة")) return "border-violet-200 bg-violet-100 text-violet-800";
