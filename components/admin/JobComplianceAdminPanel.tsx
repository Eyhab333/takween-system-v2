...
export default function JobComplianceAdminPanel() {
  ...
  const [
    selectedProgress,
    setSelectedProgress,
  ] = useState<ProgressDetail | null>(null);
  ...
  const loadDocuments =
    useCallback(async () => {
      try {
        ...
        if (!token) {
          setError(
            "لم يتم العثور على توكن تسجيل الدخول",
          );
          return;
        }

        const response = await fetch(
          "/api/admin/job-compliance",
          {
            headers: {
        ...
        if (!response.ok) {
          setError(
            json?.error ||
              "تعذر تحميل مستندات الالتزام الوظيفي",
          );
          return;
        }
        ...
        setError(
          "حدث خطأ أثناء تحميل مستندات الالتزام الوظيفي",
        );
      } finally {
        setLoading(false);
      }
  ...
  async function createDocument(
    ...
    try {
      ...
      formData.set(
        "requiresAcknowledgement",
        String(
          acknowledgementControl.checked,
        ),
      );
      ...
      if (audTokens.length === 0) {
        toast.error(
          "حدّد الجمهور المستهدف أو اختر كل الموظفين",
        );
        return;
      }
  ...
  async function toggleActive(
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/job-compliance/${document.id}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type":
      ...
      toast.error(
        "حدث خطأ أثناء تحديث حالة المستند",
      );
    } finally {
      setUpdatingId(null);
    }
  ...
  async function loadProgress(
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/job-compliance/${documentId}`,
        {
          headers: {
            Authorization:
              `Bearer ${token}`,
      ...
      setSelectedProgress({
        ...
        totalTargetEmployees:
          json.totalTargetEmployees,
        acknowledgedCount:
          json.acknowledgedCount,
        pendingCount:
          json.pendingCount,
        employeeStatuses:
          json.employeeStatuses || [],
      });
    } catch (progressError) {
      console.error(progressError);

      toast.error(
        "حدث خطأ أثناء تحميل تفاصيل الإقرارات",
      );
    } finally {
      setProgressLoadingId(null);
    }
  ...
  const filteredEmployees =
    selectedProgress?.employeeStatuses.filter(
      (employee) => {
        if (
          employeeFilter === "all"
        ) {
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            إضافة مستند التزام وظيفي
          </CardTitle>

          <CardDescription>
            يتم رفع الملف بصيغة PDF
            وتحديد الموظفين المستهدفين
            بالمستند.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          ...
          >
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-2 rounded-md border px-4 py-3 text-sm">
                <input
            ...
            <div className="grid gap-4 border-t pt-5 md:col-span-2">
              <div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              المستهدفون:{" "}
              {
                selectedProgress.totalTargetEmployees
              }
              {" · "}
              تم الإقرار:{" "}
              {
                selectedProgress.acknowledgedCount
              }
              {" · "}
              بانتظار الإقرار:{" "}
              {
                selectedProgress.pendingCount
              }
            </CardDescription>
          </CardHeader>
